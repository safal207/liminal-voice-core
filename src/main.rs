...
fn main() {
    ...
    if cfg.sync {
        ...
        if astro_drift_bias != 0.0 || astro_res_bias != 0.0 {
            consolidate_astro(astro_drift_bias, astro_res_bias);
        }
    }
...
fn consolidate_astro(drift_bias: f32, res_bias: f32) {
    println!(
        "[astro] consolidate drift_bias={:.3} res_bias={:.3}",
        drift_bias, res_bias
    );
}
